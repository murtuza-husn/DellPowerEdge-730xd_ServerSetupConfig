...
The yellow/orange light on Port 2 and the "100 Mbps" readout mean the connection is "Fast Ethernet" instead of "Gigabit Ethernet."Since you plugged Port 2 into the server's main NIC (LOM1) but did not power on the server, the server's main network card is only receiving low-voltage standby power. While in standby, most server network cards drop their link speed down to 100 Mbps to save power. Once you press the physical power button on the server and the operating system boots up, the light will turn green, and the router interface will automatically update Port 2 to 1000 Mbps.(Note: If it stays at 100 Mbps even after powering on, you have a damaged or outdated Ethernet cable that needs to be replaced with a Cat6 cable).

Even though Port 1 shows a green light and a solid 1000 Mbps connection, it is common for the iDRAC not to show up in the router's "Connected Devices" list right away for two main reasons:Static IP Conflict: If the previous owner of the server configured a static IP inside the iDRAC (such as the Dell factory default 192.168.0.120), the iDRAC will completely ignore your Rogers router's DHCP pool. Because it isn't asking for an IP, the Rogers router doesn't index it in the active device table.Arp Table Delay: The router's "Connected Devices" page updates slowly. The iDRAC may be awake, but it hasn't broadcasted any outward traffic yet to make its presence known to the page.How to fix this and find it:Since you cannot see it on the page, use Method 1 from our previous discussion: Open a browser on a laptop connected to the same Wi-Fi, look at the physical luggage tag sticker on the server for the 7-character Service Tag (e.g., ABC1234), and type https://idrac-ABC1234.local into your URL bar. This forces your computer to search the local network directly for the iDRAC hardware, bypassing the router's blind spot.

How to fix this and find it:Since you cannot see it on the page, use Method 1 from our previous discussion: Open a browser on a laptop connected to the same Wi-Fi, look at the physical luggage tag sticker on the server for the 7-character Service Tag (e.g., ABC1234), and type https://idrac-ABC1234.local into your URL bar. This forces your computer to search the local network directly for the iDRAC hardware, bypassing the router's blind spot.
